...
module.exports.execute = async (client, message, args) => {
  ...
  else return await message.channel.send(':x: Must include a title surrounded in quotes.')
  ...
  if (suggestDesc.length > 1000) return await message.channel.send(':x: Suggestions must not be longer than one thousand characters.');
...
**Please note that this message will disappear in 20 seconds.**`
	...
	confirmation_message.awaitReactions(filter, { max: 1, time: 20000, errors: ['time'] })
		...
		.catch(err => {
			confirmation_message.delete();
      return message.reply('You didn\'t confirm nor deny. Please try again or use `.suggest help` for guidance!');
		});
}

async function postReminder(message) {
  var suggestion_channel = message.guild.channels.cache.get(config.channel.suggestions);
  ...
  if (suggestion_channel) {
    var suggestEmbed = new Discord.MessageEmbed()
      ...
      .setFooter(`Suggestion by ${message.author.username}#${message.author.tag} on ${day}/${month}/${year}.`);
