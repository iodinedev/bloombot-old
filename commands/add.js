  ...
  if (message.channel.id === config.channels.meditation || message.channel.id === config.channels.group_meditation) {
    ...
    if (time !== NaN && time <= 600) {
        ...
        try {
            ...
            var add_lvl_role = await member.guild.roles.cache.find(role => role.id === config.roles.lvl_roles[lvl_role]);
            ...
            var add_streak_role = await member.guild.roles.cache.find(role => role.id === config.roles.streak_roles[streak_role]);
